...
export default function Projet({name,picture,techno,description,buttons}:projetProps) {
  const showButton=(type:string,link:string)=>{
    ...
    <Link to={link}  target="_blank">
     <button type="button" className="text-white bg-secondary hover:bg-[#050708]/80 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:hover:bg-[#050708]/40 dark:focus:ring-gray-600 me-2 mb-2">
    <svg className="w-5 h-5 me-2 -ms-1" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="max-w-sm rounded overflow-hidden shadow-lg sm:hover:-translate-y-2 h-auto">
        ...
        <div className="px-6">
          {techno.map((tech,index)=>(
             <Link to={`${tech.url}`} target="_blank" rel="noopener noreferrer" key={index}>
             <span className="inline-block bg-gray-200 hover:bg-secondary rounded-sm px-3 py-1 text-xs font-semibold text-gray-700 hover:text-white  mr-2 mb-2">#{tech.name}</span>
           </Link>

          ))}
          <p className='text-gray-500 text-xs max-w-[300px] mt-4'>{description}</p>
       
          <div className="button-elements mt-2">
{
 buttons.map((button)=>(
  showButton(button.type,button.link)
 ))
